...
from utils.utils import test_GDN, test_sage, load_data, pos_neg_split, normalize, biased_split, sparse_to_adjlist
...
class ModelHandler(object):
	...
	def __init__(self, config):
		...
		feat_data, labels = load_data(args.data_name, prefix=args.data_dir)
		...
		if args.model == 'SAGE' or args.model == 'GCN':
			...
			adj_lists = [homo] # for tfinance/tsocial
		...
		for epoch in range(args.num_epochs):
			...
			for batch in range(num_batches):
				...
				if args.add_constraint:
					...
					feat_c = feat_data[:, grads_idx]
					np.save(r'./data/tsocial_feat_c8.npy', feat_c)
					...
					for i in range(feat_data.shape[1]):
						if i not in grads_idx:
							non_grads_idx[idx] = i
							idx += 1
					feat_s = feat_data[:, non_grads_idx]
					np.save(r'./data/tsocial_feat_s8.npy', feat_s)  # ***********保存class feature S************
