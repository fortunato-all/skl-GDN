...
import random
from models.sinkhorn import getSinkhornLoss
...
class InterAgg(nn.Module):
	...
	def __init__(self, features, feature_dim, embed_dim, 
		...
		self.cos0 = nn.CosineSimilarity(dim=0, eps=1e-6)
	...
	def fl_loss(self, grads_idx):
		"""
		:param grads_idx
		:return: Skl(Cv,proto+),Skl(Cv,proto-)
		"""
		x = self.features(self.pos_index)[:, grads_idx].softmax(dim=-1) 
		...
		# target_pos = F.log_softmax(self.pos_vector[:, grads_idx].repeat(x.shape[0], 1), dim=-1)
		# target_neg = F.log_softmax(self.neg_vector[:, grads_idx].repeat(x.shape[0], 1), dim=-1)

		cost_pos = torch.zeros(len(grads_idx),len(grads_idx))
		for i in range(len(grads_idx)):
			for j in range(len(grads_idx)):
				cost_pos[i,j] = 1-self.cos0(x[:,i],target_pos[:,j])
		cost_neg = torch.zeros(len(grads_idx),len(grads_idx))
		for i in range(len(grads_idx)):
			for j in range(len(grads_idx)):
				cost_neg[i, j] = 1-self.cos0(x[:, i], target_neg[:, j])

		skloss_p = getSinkhornLoss(cost_pos, 1.0, 3)
		skloss_n = getSinkhornLoss(cost_neg, 1.0, 3)
		loss_pos = skloss_p(target_pos, x)
		loss_neg = skloss_n(target_neg, x)
...
class IntraAgg(nn.Module):
	...
	def fn_loss(self, non_grad_idx, target, neighs, all_nodes, all_neighs):
		x = self.fetch_feat(target)[:, non_grad_idx].softmax(dim=-1)
		...
		# loss_pos = self.KLDiv(x, pos) #kl
		# loss_neg = self.KLDiv(x, neg) #kl

		cost_pos = torch.zeros(len(non_grad_idx), len(non_grad_idx))
		for i in range(len(non_grad_idx)):
			for j in range(len(non_grad_idx)):
				cost_pos[i, j] = 1-self.cos0(x[:, i], pos[:, j])
		cost_neg = torch.zeros(len(non_grad_idx), len(non_grad_idx))
		for i in range(len(non_grad_idx)):
			for j in range(len(non_grad_idx)):
				cost_neg[i, j] = 1-self.cos0(x[:, i], neg[:, j])
		skloss_p = getSinkhornLoss(cost_pos, 1.0, 3)
		skloss_n = getSinkhornLoss(cost_neg, 1.0, 3)
		loss_pos = skloss_p(pos, x)
		loss_neg = skloss_n(neg, x)
		# pdb.set_trace()
		return loss_pos, loss_neg
